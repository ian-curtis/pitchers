...
```{r}
# new_files <- list("0809.csv")
# 
# for (i in seq(10, 31, 1)) {
#   filename <- paste0("08", i, ".csv")
#   new_files <- append(new_files, filename)
# }
```

```{r}
for (filename in my_files[133:163]) {
  file <- read_csv(filename) %>%
    select(c(game_date:description, zone, stand:strikes, plate_x:inning_topbot, sz_top:release_spin_rate, at_bat_number:fld_score))

  write_csv(x = file, file = filename)
